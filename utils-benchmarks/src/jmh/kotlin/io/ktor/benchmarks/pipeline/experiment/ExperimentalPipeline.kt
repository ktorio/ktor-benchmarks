 ...
 */
open class ExperimentalPipeline<TSubject : Any, TContext : Any>(
     ...
     */
    fun merge(from: ExperimentalPipeline<TSubject, TContext>) {
    ...
    private fun cacheInterceptors(): List<PipelineInterceptor<TSubject, TContext>> {
        ...
        val destination: MutableList<suspend PipelineContext<TSubject, TContext>.(TSubject) -> Unit> = mutableListOf()
        for (phaseIndex in 0..phases.lastIndex) {
            @Suppress("UNCHECKED_CAST")
    ...
    private fun fastPathMerge(from: ExperimentalPipeline<TSubject, TContext>): Boolean {
    ...
    private fun setInterceptorsListFromAnotherPipeline(pipeline: ExperimentalPipeline<TSubject, TContext>) {
...
suspend inline fun <TContext : Any> ExperimentalPipeline<Unit, TContext>.execute(
    context: TContext
 ...
 */
inline fun <reified TSubject : Any, TContext : Any> ExperimentalPipeline<*, TContext>.intercept(
    phase: PipelinePhase,
