...
class GitSource:
    def __init__(self, commit):
        try:
            subprocess.check_output(
                ["git", "rev-parse", "--verify", commit], cwd=REPO, stderr=subprocess.DEVNULL
            )
        except subprocess.CalledProcessError:
            raise ValueError(f"Invalid git ref: {commit!r}") from None
        self.commit = commit
    ...
    def list_files(self, subdir):
        ...
        try:
            out = subprocess.check_output(
                ["git", "ls-tree", "--name-only", f"{self.commit}:{git_dir}"], cwd=REPO
            ).decode()
        except subprocess.CalledProcessError:
            return []
    ...
    def load(self, subdir, fname):
        ...
        raw = subprocess.check_output(
            ["git", "show", f"{self.commit}:{git_dir}/{fname}"], cwd=REPO
        )
    ...
    def load_sites(self, scenario, required=True):
        ...
        result = subprocess.run(
            ["git", "show", f"{self.commit}:{path}"], cwd=REPO, capture_output=True
        )
        if result.returncode != 0 or not result.stdout.strip():
            if required:
                raise FileNotFoundError(f"{path} not found at {self.commit!r}")
            return []
...
def local_sources():
    """Return (old, new) for local mode: allocations/ vs build/allocations/."""
